...
包含ComputeNode和NodeManager的完整实现。
"""
...
class ComputeNode:
    ...
    def __init__(
        ...
        # 安全组件（模拟）
        self.security_manager = None  # 实际应该导入SecurityManager
        
        # 监控组件（模拟）
        self.metrics_collector = None  # 实际应该导入MetricsCollector
    ...
    def register_to_network(self, network_address: str) -> bool:
        ...
        try:
            # 模拟生成节点证书
            certificate = f"cert_{self.node_id}_{int(time.time())}"
            ...
            # 模拟签名注册数据
            signature = f"sig_{hashlib.sha256(json.dumps(registration_data, sort_keys=True).encode()).hexdigest()[:32]}"
    ...
    def shutdown(self):
        """关闭节点"""
        logger.info(f"节点 {self.node_id} 正在关闭")
        ...
        for task_id in list(self.current_tasks):
            self.fail_task(task_id, "节点关闭")
        
        logger.info(f"节点 {self.node_id} 已关闭")


class NodeManager:
    """节点管理器
    
    负责管理网络中的所有计算节点，包括节点发现、验证、调度等。
    """
    ...
    def register_node(self, node_data: Dict) -> Optional[str]:
        """
        注册新节点